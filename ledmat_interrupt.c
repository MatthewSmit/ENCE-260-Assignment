...
void ledmat_display_column (uint8_t pattern, uint8_t col)
{
    ...
    /* Disable previous columns to prevent ghosting while rows modified.  */
	/* For some reason, ghosting appears much more frequent with interrupts enabled. Just clear them all. */
